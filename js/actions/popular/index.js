...
export function onLoadPopularData(storeName, url, pageSize, favoriteDao) {
	return (dispatch) => {
		...
		dataStore
			.fetchData(url, FLAG_STORAGE.falg_popular) //异步action与数据流
			.then((data) => {
				handleData(actionTypes.POPULAR_REFRESH_SUCCESS, dispatch, storeName, data, pageSize, favoriteDao);
			})
			.catch((err) => {
				console.log(err);
				dispatch({
...
export function onFlushPopularFavorite(storeName, pageIndex, pageSize, dataArray = [], favoriteDao) {
	return (dispatch) => {
		...
		_projectModels(dataArray.slice(0, max), favoriteDao, (data) => {
			dispatch({
				type: actionTypes.FLUSH_POPULAR_FAVORITE,
				storeName,
				pageIndex,
				projectModels: data
			});
